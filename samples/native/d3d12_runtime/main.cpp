...
int32_t main(int32_t argc, char **argv) {
	...
	// create window
	String title = String::format("%s Tellusim::D3D12Runtime", window.getPlatformName());
	...
	if(!pipeline.loadShader(Shader::TypeVertex, "main.hlsl", "VERTEX_SHADER=1")) return 1;
	if(!pipeline.loadShader(Shader::TypeFragment, "main.hlsl", "FRAGMENT_SHADER=1")) return 1;
	...
	window.run([&]() {
		...
		target.begin();
		{
			// create Direct3D12 command list
			...
			// update Direct3D12 command
			command.update();
			
			// draw model with Direct3D12 API
			for(uint32_t i = 0; i < model.getNumGeometries(); i++) {
				...
				command.getD3D12Command()->DrawIndexedInstanced(num_indices, 1, base_index, base_vertex, 0);
			}
		}
		target.end();
