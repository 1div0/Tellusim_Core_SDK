...
int32_t main(int32_t argc, char **argv) {
	...
	// create controls
	ControlSlider steps_slider(&panel, "Steps", 4, 1, 6);
	steps_slider.setSize(192.0f, 0.0f);
	
	ControlSlider weight_slider(&panel, "Weight", 3, 1.0f, 0.0f, 1.0f);
	weight_slider.setSize(192.0f, 0.0f);
	...
	ControlSlider::ChangedCallback changed_func = ControlSlider::ChangedCallback([&](ControlSlider slider) {
		...
		MeshRefine::subdiv(subdiv_mesh, src_mesh, steps_slider.getValueu32(), weight_slider.getValuef32());
		...
		// subdiv model
		if(subdiv_model.create(device, pipeline, subdiv_mesh)) {
			String info;
			...
			info += String::format("Indices: %u -> %u", src_model.getNumIndices(), subdiv_model.getNumIndices());
	...
	steps_slider.setChangedCallback(changed_func);
	weight_slider.setChangedCallback(changed_func);
