...
int32_t main(int32_t argc, char **argv) {
	...
	if(!kernel.loadShaderGLSL("main.shader", "COMPUTE_SHADER=1; NUM_BUFFERS=%u", num_buffers)) return 1;
	if(!kernel.create()) return 1;
	
	// create buffers
	Array<Buffer> buffers;
	Array<Vector4u> index_data(num_buffers);
	Array<uint32_t> buffer_data(buffer_size, 0u);
	for(uint32_t i = 0; i < num_buffers; i++) {
		index_data[i].x = num_buffers - i - 1;
	...
	
	{
		...
		compute.setUniformData(0, index_data.get(), index_data.bytes());
	...
	for(uint32_t i = 0; i < buffers.size(); i++) {
		if(!device.getBuffer(buffers[index_data[i].x], buffer_data.get(), buffer_data.bytes())) return 1;
		for(uint32_t j = 0; j < buffer_data.size(); j++) {
			if(buffer_data[j] != i + j) TS_LOGF(Fatal, "%u %u: %u %u\n", i, j, buffer_data[j], i + j);
			data += buffer_data[j];
		}
	}
	TS_LOGF(Message, "%u\n", data);
