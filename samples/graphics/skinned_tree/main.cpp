...
int32_t main(int32_t argc, char **argv) {
	...
	// create window
	String title = String::format("%s Tellusim::SkinnedTree", window.getPlatformName());
	...
	// create instance kernel
	Kernel instances_kernel = device.createKernel().setUniforms(1).setStorages(3);
	if(!instances_kernel.loadShaderGLSL("main.shader", "COMPUTE_INSTANCES_SHADER=1; GROUP_SIZE=%uu; NUM_INSTANCES=%uu", group_size, num_instances2)) return 1;
	if(!instances_kernel.create()) return 1;
	...
	if(!vertex_model.create(device, vertex_pipeline, geometry, MeshModel::DefaultFlags | MeshModel::FlagIndices32 | MeshModel::FlagBufferStorage)) return 1;
	Buffer model_vertex_buffer = vertex_model.getVertexBuffer();
	Buffer model_index_buffer = vertex_model.getIndexBuffer();
	...
	if(!radix_sort.create(device, RadixSort::FlagsAll, prefix_scan, num_triangles * 2, group_size, num_instances2)) return 1;
	if(!spatial_tree.create(device, SpatialTree::FlagsAllOptimize, radix_sort, num_triangles * 2, group_size, num_instances2)) return 1;
	...
	Buffer vertex_buffer = device.createBuffer(Buffer::FlagStorage, sizeof(Vertex) * num_vertices * num_instances2);
	Buffer bottom_buffer = device.createBuffer(Buffer::FlagStorage, sizeof(SpatialTree::Node) * num_triangles * 2 * num_instances2);
	Buffer top_buffer = device.createBuffer(Buffer::FlagStorage | Buffer::FlagSource, sizeof(SpatialTree::Node) * num_instances2 * 2);
	if(!joints_buffer || !vertex_buffer || !bottom_buffer || !top_buffer) return 1;
	...
	window.run([&]() {
		...
		if(animation_check.isChecked() || frame_counter == 0) {
			...
			float32_t scale = pow(min(9.0f / num_instances, 1.0f), 0.9f);
			...
			for(uint32_t i = 0, y = 0; y < num_instances; y++) {
				for(uint32_t x = 0; x < num_instances; x++, i++) {
					if(i < 128) {
						...
						animation.setTime(animation_time * 0.7 + random.getf32(3.0f, 9.0f), Matrix4x3d::scale(Vector3d(scale)));
		...
		// spatial tree
		{
			...
			NodesParameters nodes_parameters;
			nodes_parameters.num_vertices = num_vertices;
			nodes_parameters.num_triangles = num_triangles;
			nodes_parameters.num_instances = num_instances2;
			...
			compute.setUniform(0, nodes_parameters);
			compute.setStorageBuffer(0, bottom_buffer);
			compute.setStorageBuffer(1, vertex_buffer);
			compute.setStorageBuffer(2, model_index_buffer);
			compute.dispatch(num_triangles * num_instances2);
			compute.barrier(bottom_buffer);
			
			if(build_query) compute.beginQuery(build_query);
			
			// dispatch spatial tree
			Array<uint32_t> dispatch_sizes;
			Array<uint32_t> dispatch_offsets;
			for(uint32_t i = 0; i < num_instances2; i++) {
				dispatch_offsets.append(num_triangles * i * 2);
				dispatch_sizes.append(num_triangles);
			}
			SpatialTree::Flags spatial_flags = SpatialTree::FlagNone;
			if(update_check.isChecked()) spatial_flags |= SpatialTree::FlagUpdate;
			spatial_tree.dispatch(compute, SpatialTree::HashXYZ10, bottom_buffer, num_instances2, dispatch_offsets.get(), dispatch_sizes.get(), spatial_flags);
			
			if(build_query) compute.endQuery(build_query);
			...
			// create instances bounds
			compute.setKernel(instances_kernel);
			compute.setUniform(0, num_triangles);
			compute.setStorageData(0, transforms.get(), transforms.bytes());
			compute.setStorageBuffer(1, bottom_buffer);
			compute.setStorageBuffer(2, top_buffer);
			compute.dispatch(num_instances2);
			compute.barrier(top_buffer);
			
			// dispatch spatial tree
			spatial_tree.dispatch(compute, SpatialTree::HashXY10, top_buffer, 0, num_instances2);
		}
		...
		// raytracer
		{
			...
			compute.setStorageData(0, itransforms.get(), itransforms.bytes());
			compute.setStorageBuffer(1, top_buffer);
			compute.setStorageBuffer(2, bottom_buffer);
			compute.setStorageBuffer(3, vertex_buffer);
			compute.setStorageBuffer(4, model_index_buffer);
