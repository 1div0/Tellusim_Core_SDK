 ...
 */
	...
	void main() {
		...
		nodes_buffer[global_id + num_lights].bound_min = vec4(position.xyz - position.w, uintBitsToFloat(1u << 31u));
		nodes_buffer[global_id + num_lights].bound_max = vec4(position.xyz + position.w, 0.0f);
	}
