...
class Application {
	...
	dispatch(compute) {
		...
		if(this.surface === undefined || this.surface.width != width || this.surface.height != height) {
			if(this.surface !== undefined) this.device.releaseTexture(this.surface);
			this.surface = this.device.createTexture2D(ts.Format.RGBAu8n, width, height, ts.Texture.Flags.Surface);
			if(!this.surface.isValidPtr()) {
				ts.Log.print(ts.Log.Level.Error, 'Application::dispatch(): can\'t create Texture\n');
				return false;
			}
		}
		
		// compute uniform parameters
		let parameters_buffer = new ArrayBuffer(4);
		let parameters = new Float32Array(parameters_buffer);
		parameters.set([ ts.Time.seconds() ]);
		
		// dispatch kernel
		compute.setKernel(this.kernel);
		compute.setUniform(0, parameters);
		compute.setSurfaceTexture(0, this.surface);
		compute.dispatch(this.surface);
		compute.barrier(this.surface);
	...
	render(command) {
		...
		command.setTexture(0, this.surface);
	...
	create_resources() {
		...
		// create kernel
		this.kernel = this.device.createKernel().setSurfaces(1).setUniforms(1);
		if(!this.kernel.loadShaderGLSL('main.shader', 'COMPUTE_SHADER=1')) {
			ts.Log.print(ts.Log.Level.Error, 'Application::create_resources(): can\'t load Compute shader\n');
			return false;
		}
		if(!this.kernel.create()) {
			ts.Log.print(ts.Log.Level.Error, 'Application::create_resources(): can\'t create Kernel\n');
			return false;
		}
		
		// create pipeline
		this.pipeline = this.device.createPipeline();
		this.pipeline.setTextureMask(0, ts.Shader.Mask.Fragment);
	...
	render_window() {
		...
		// compute target
		{
			let compute = this.device.createCompute();
			
			// dispatch application
			if(!this.dispatch(compute)) return false;
			
			compute.destroyPtr();
		}
		
		// flush surface texture
		this.device.flushTexture(this.surface);
